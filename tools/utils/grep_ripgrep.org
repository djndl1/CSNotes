...
-To grep against a hyphen, quote the pattern and surround the hyphen with a pair of brackets: =grep '[-]i'=.
  + or use =-e= to explicitly specify the following token is a pattern.
