...
- =M-g c=, =goto-char=: to the nth char in the buffer
  + not of much use (probably only in diagostics)
...
- =C-h a=: searches for commands =aprops-command=
...
- =isearch-forward=, =isearch-backward= :: the search ends with a =RET= (=isearch-exit=) when the
  ...
  + =M-s h r= (=highlight-regexp=) leaves the search while leaving the matches highlighted.  
    =M-s h u= (=unhighlight-regexp=) remoes the highlight. These two can be used independent of
    isearch. 
  + =RET= with no search strings starts */nonincremental search/* (not of much
    use and inconvenient to use), or with
    =(re-)search-forward= and =(re-)search-backward=.
  + =M-s M->= and =M-s M-<= goes to the last/first occurence.
