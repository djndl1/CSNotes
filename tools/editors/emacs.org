...
- *Register*: where positions, text, rectangles, numbers, windows configurations,
  file names are saved for later use.
  + named after a alphanumberic character (case-sensitive).
  + Unlike Vim, Emacs has both registers and kill rings.
...
Saving position for a later jump. Like Vim's marks, not the Emacs' mark ring.

- =C-x r SPC RegisterName=: =point-to-register= record the position of point and the current buffer in register =RegisterName=

- =C-x r j RegisterName=: =jump-to-register= =RegisterName=

** Saving Text

- =C-x r s RegisterName=: =copy-to-register=, copies the text of the region into the register.

- =append-to-register=

- =prepend-to-register=

- =C-x r i RegisterName=: =insert-register=: insert the content of =RegisterName= at point

** Saving Rectangles

- =C-x r r RegisterName=: =copy-rectangle-to-register=

- =C-x r i RegisterName=: =insert-register=

** Saving File/Buffer Names

- =(set-register R '(file . Name))=

- then run =C-x r j R= to jump to the file.

** Saving Keyboard Macros

- =kmacro-to-register= (=C-x C-k x=) saves the current keyboard macro for later use.

** Bookmarks

Persisted position records for later restoration. There is a default bookmark file
(=bookmar-default-file=) that various bookmark commands use to save bookmarks to.

- =C-x r m RET= (=bookmark=set=): set the bookmark for the visited file at point.

- =C-x r b bookmark RET=: =bookmark-jump= jump to the bookmark
...
*** =flymake-mode= (Builtin-in) On-The-Fly Syntax Checking (Make On-The-Fly to Check)
...
It periodically compiles an active buffer when the minor mode is enabled, and then flags or
highlights lines that cause errors or warnings during the compile as you edit.
#+end_quote

Aside from various primitive backends, it now integrates with eglot to provide LSP diagnostics.

*** Usage

- =flymake-show-buffer-diagnostics=, =flyamek-show-project-diagnostics= or click the left fringe or
  the mode line.
...
- /merge-based/ and /lock-based/: how the VCS handles concurrent modification on
  ...
  + Older lock-based systems use the terms "check in" (commit) and "check out" (update).
...
- /Changeset-based/ and /file-based/: how changes are organized as a unit. Newer
  ...
  + SCCS, RCS, CVS and other early VCS handles changes per file.
  + Changeset-based version control is more flexible and powerful than file-based version control.
...
- =vc-next-action= (=C-x v v=): confusing command but unavoidable.
  + a multi-purpose command, the most appropriate action on the current VC fileset:
    registering, committing (=*vc-log*=)
  + in a file-visiting buffer, in a Dired buffer or in a VC directory buffer.

- =vc-register= (=C-x v i= =git add=): register the file in the repository

- =vc-diff= =C-x v ==, =vc-ediff=, =vc-root-diff= (=C-x v D= compare the entire working tree): 
  compare two revisions. Without a prefix argument, it defaults to the work files and the last
  revision.

- =vc-revision-other-window=: show a revision (commit) of the current file in another
  window

- =vc-annotate= (=C-x v g= =git annotate/blame=): for each line, show the latest revision in which it was
  modified.

- =vc-print-log= (=C-x v l= the current fileset), =vc-print-root-log= (=C-x v L= the current
  repository), =vc-print-branch-log= (=C-x v b l= a certain branch),
  =vc-log-incoming= (compared with new changes made by a pull operation),
  =vc-log-outgoing= (changes that will be sent by a push operation),
  =vc-region-history= (with the history of changes made in the region)
...
- =project-find-dir= (=C-x p d=): it really just finds a directory

- =project-find-regexp= (=C-x p g=): search for a pattern in all files in the project

- =project-query-replace-regexp= (=C-x p r=)

- =project-search=:

- =project-dired= (=C-x p D=): open dired in the current project

- =project-vc-dir=:

- =project-eshell=, =project-shell=:

- =project-compile= (=C-x p c=): run a compilation commands

- =project-shell-command=, =project-async-shell-command=:

- =project-switch-to-buffer= (=C-x p b=): switch an open buffer in the project

- =project-list-buffers= (=C-x p C-b=): list buffers that belong to the project

- =project-kill-buffers=: kill all live buffers that belong to the current project

- =project-switch-project= (=C-x p p=)

- =project-forget-project=: remove a project from =project-list-file=

- =project-any-command= any Emacs command in the current project  

** Emacs Development Environment

Not used anymore with LSP, mostly used for C/C++, not recommended.
...
host editing and related operations.
#+end_quote

Initiate =find-file= and enter =/method:user@host:/path/to/file=.
...
- *inline methods*: use the same login connection to transfer file contents (dependening on some
  external encoding programs =tramp-remote-coding-commands=).
  + quick and easy for small files
  + ssh, telnet, su, sudo, plink, sshx (ssh with special commands to force a pseudoterminal),
    docker, podman, kubernetes

- *external methods*: operate over multiple channels
  + rcp, scp, rsync, fcp, ftp, smb (=$$= for =$= in paths, =user%domain@host=), nc (telnet + nc, for routers or NAS hosts)
  + GVFS (=tramp-gvfs-enabled=, =tramp-gvfs-methods=): atp, dav(s), gdrive, mtp, nextcloud, sftp
  + FUSE: sshfs, rclone
  + dockercp, podmancp, kubernetes, magit, hdfs, vagrant

*** =su=, =sudo= and Others

TRAMP is not only used for remote connections.

*** Remote Processes (e.g. Language Server)

TRAMP supports starting new running processes on the remote host for discovering remote file
names (not for every connection method: GVFS, ftp).
Emacs packages on the remote host need no specific modifications for TRAMP’s use.

- =tramp-remote-path=: remote executable paths for TRAMP (not the real =PATH= environment variable)
  
- =winexe= (part of Samba) is used to run remote Windows processes. The remote host needs to have
  PowerShell 2.0.
