...
- /Be clear/: Write clearly, don't be too clever.
  + The goal is to write clear code, not clever code. It is more important to make the purpose
    of the code unmistakable than to display virtuosity. Don't make debugging and modification
    more difficult.
  ...
  #+begin_src fortran
! one may be tempted to create an identity matrix like  
        do i = 1, N
          do j = 1, N
              V(i, j) = (i / j) * (j / i)
! unless i equals j, the result is zero, otherwise one
          enddo
        enddo
! it looks clever but not very easy to understand, also 
! performance-wise, it requires two divisions and one multiplication.
! a 
        do i = 1, N
          do j = 1, N
              V(i, j) = 0.0 ! zero the row
          V(i, i) = 1.0     ! make the one at the diagonal
          enddo
        enddo
  #+end_src

- turn relational tests around if it makes the program easier to understand
...
- (C/C++) Define numbers as constants, not macros 
  + for C, one can use =enum= for integer constants.

- (C/C++) Use character constants, not integers.

- (C/C++) use the language to calculate the size of an object
  + =sizeof= and =(sizeof(array) / sizeof(array[0])=

* Comments

#+begin_quote
They do not help by saying things the code already plainly says, 
or by contradicting the code, or by distracting the
reader with elaborate typographical displays.
#+end_quote
...
- Do not belabor the obvious:
  ...
  + Comments should explain the code or complement the code.
