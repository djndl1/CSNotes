...
conversion is affected by the locale. The problem that is often ignored is that
on startup, C programs set the locale to ="C"=, which under Linux restricts the
charset to ASCII (at least with glibc).
Non-ASCII characters are ignored unless the target charset is able to represent
them. Simply setting =LC_TYPE= as an environment variable won't work unless
=setlocale()= is called with that value. Also, the locale in use has to be
generated beforehand with =locale-gen=.
