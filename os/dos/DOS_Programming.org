...
A DOS extender enables software to run in a protected mode environment even the OS is only
...
hardware support but it does not require protected mode support. Expanded
memory emphasizes the use of more memory without necessarily adding more address space.
...
mode or virtual x86 mode. Extended memory emphasizes the use of higher memory addresses.
