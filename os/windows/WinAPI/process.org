...
#+BEGIN_SRC C
  void ExitProcess(UINT uExitCode); // DLL detach calls are made, termination handlers are not executed, same as return from main function
  BOOL GetExitCodeProcess(HANDLE  hProcess, LPDWORD lpExitCode);
  BOOL TerminateProcess(HANDLE hProcess, UINT   uExitCode); // Neither SEH nor DLLMain functions are executed
#+END_SRC

=__finally= and =__except= are not executed when =ExitProcess= is called. Console control handlers are a limited alternative to signal another process to cleanly shutdown.
