...
If A different MSVC Redistributable 2010 is installed, it should be uninstalled so that SDK 7.1 installer may run.
...
No good source line debuggers exist for modern PDB files. Nor is it possible 
to remote debug on Windows XP. =mdbg= works at CIL level.

Strangely, programs built by =dotnet build= on a samba host requires full =rwx= to run 
over SMB by XP.
