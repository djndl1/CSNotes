> What every programmer would like is a private, infinitely large, infinitely fast memory that is also nonvolatile and inexpensive

The solution to this dream is memory hierarchy:  It is the job of the operating system (*memory manager*) to abstract this hierarchy into a useful model and then manage the abstraction.


# Early Memory Management: No Memory Abstraction

Memory was still divided into regions (for OS, for device drivers and for user programs).

Every program simply saw the physical memory. A program may have multiple threads, but it cannot have multiple unrelated programs running together,
which is not of much use because people tend to want unrelatedp programs to run concurrently.

## Multiprogramming without Memory Abstraction

- *swapping*: One way to run multiple programs is to swap the one in memory to disk and load another process into memory. 

- *keyed protection storage*:  IBM 360 segmented memory into 2-KB blocks, each of which is assigned a protection key, 
  ...
  - reintroduced into [Memory protection keys](https://lwn.net/Articles/643797/) for security enhancement

# Address Space: Abstract Memory for Processes

_Protection_ and _relocation_ are the two major problems with running multiple programs in memory. 
Some form of memory abstraction has to be made for multiprogramming. Keyed storage blocks were enough for address relocation.
...
- _address space_: the set of addresses that a process can use to address memory.
   - Address space separates the memory space the process sees from the physical memory by providing a unique address space to each process,
     creating a kind of *abstract memory*. The harder part is how:

- *the base-and-limit approach*: a simple version of *dynamic relocation*. A program are loaded into consecutive memory location. The base register is loaded with the physical address where its program begins in memory and the limit register is loaded with the length of the program. Dynamic relocation is used. Every time a process references memory, the CPU hardware automatically adds the base value to the address generated by the process before sending the address out on the memory bus. Only the OS can modify the base and limit registers. This gives each process a private memory space.
  ...
  - dynamic relocation requires an addition and a comparison on every memory

However, not all programs can be stuffed into main memory. Two general strategies exist:

- _(full) swapping_: consists of bringing in each process in its entirety, running it for a while, then putting it back on the disk. 
  ...
  + adding a little extra memory for growth may not be enough for many programs. Even if we can have the heap and the stack (supposedly only one thread) at two ends of the reserved memory region, the region could be filled up.
...
- _bitmaps_: memory is divided into allocation units with each unit assigned a bit to indicate its allocation state. 
