...
def inverted_bottleneck(filters, strides=1, expansion_factor=6, regularizer=keras.regularizers.l2(4e-5)):
    canary = 1 if keras.backend.image_data_format() == 'channels_first' else 3

    def bottleneck(x):
        input_channels = K.int_shape(x)[canary]
...
def build_mobilenet(width_multipler=1):
    ...
    for _ in range(1):
        conved = inverted_bottleneck(16)(conved)

    conved = inverted_bottleneck(24, strides=2)(conved)
    for _ in range(1):
        conved = inverted_bottleneck(24, strides=1)(conved)

    for _ in range(3):
        conved = inverted_bottleneck(32, strides=1)(conved)

    conved = inverted_bottleneck(64, strides=2)(conved)
    for _ in range(3):
    ...
    for _ in range(2):
        conved = inverted_bottleneck(160, strides=1)(conved)

    for _ in range(1):
        conved = inverted_bottleneck(320)(conved)

    conved = ConvBNBlock(1380, (1, 1))(conved)
    ...
    sgd = keras.optimizers.SGD(lr=0.045, momentum=0.05)
