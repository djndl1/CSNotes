...
previous one, or an inner variable shadows the outer one.
...
- /variable (binding) mutability/: Whether a variable cannot be associated with another value
  (binding mutability, Java =final=).
  
- /value mutablity/: whether the object's internal state allow change.

In Rust, mutability is a property of the access path, not an intrinsic property of the type.

- An imumutable binding means the value cannot be modified through this variable; nor can one create
  a mutable reference to it. Nor this variable can rebound: a binding immutability that prevents
  value mutablity.

- Reference mutability also limits the mutability of the access path. A binding can be mutable,
  but if it refer to an immutable reference, it can be rebound but the value is still immutable.
  Mutability is part of reference type, not of the variables or the object.

Rust's default and primary mechanism is binding mutability. But this binding mutability gatekeeps 
whether value mutability is allowed (/inherited mutability/). Unlike Java, a =final= variable holding an object cannot
prevent the user from calling a mutating method on the object. In Rust, a mutable binding (=let mut=
or =&mut=) is required to mutate the value itself. =let mut= and =let= are about binding mutability,
but a immutable variable can contain a mutable reference to a value. Exclusive access rule in Rust
aside, it is like:

#+begin_src c
const object *mutable_ptr_to_immutable;   // a mutable pointer variable pointing to an immutable object
object *immutatable_ptr_to_mutable const;                // an immutable poiner variable pointing to a mutable object
#+end_src

Furthermore, =Cell=, =RefCel=, =Rc=, =Arc= etc form /interior mutability/ (object mutability made
safe). These types are provided to overcome the limitations of the compiler in a controller manner
(mutable via =&T=, when commonly inhertied mutability is used via =&mut T=). Brorows for
=RefCell= are tracked at runtime, unlike typical staticallyed traced refrence types.
...
- Each value has a variable called its /owner/, but only one at a time
  and the value is /dropped/ (=Drop= is called) as soon as the owner goes out of scope.
...
- A /string slice/ is a reference to part of a =String=:
  ...
  + string literals are referred to as string slices. Using string slices as parameter types is
    better than a =String&= type.
  + string slicing is a reference to a string and remains valid, subject to borrow rules and compiler check and is why 
    it is better than a pair of indices.
...
#+BEGIN_SRC rust
// implicit relation here has s bound to the returned slice without lifetime annotation
// fn fist_word(s: &'a String) -> &'a str by default
...
- =Debug= pretty-print trait output:
