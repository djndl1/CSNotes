...
- support for /concurrent programming/, include =thread=s and locks;
...
We can do formatting to and from memory using =stringsteam=s.
...
In addition, container adpators =queue<T>=, =stack<T>= and
...
repeated =push_back()=s is efficient.
...
Many performance-critical applications use =vector=s and checking all
...
there is a compelling reason, use =vector=s, which perform better for
...
using =()=. =function=s are useful for callbacks, for passing operations
...
=static_assert=s, compile-time =if=s, and =enable_if=s. The
...
condition\_variables found in =<condition_variable>=.
...
- =packaged_task= to help launch tasks and connect up the mechanisms for
  ...
  connected with =future=s and =promise=s to be run on =thread=s. A
