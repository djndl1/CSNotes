In C++ the multi-threading implementation offered by the language is a
...
- =std::lock_guard=: basic =unlock_guarantee=, a basic wrapper. This
...
- =fetch_add=; =fetch_sub=; =fetch_and=; =operator+==, =operator-==,
