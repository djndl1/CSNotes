...
- Use a =KeyPress= to limit what keys the user may input.
  ...
  There is no buitin =NumericUpDown= in VB6, one could 
...
that will lose focus. A =False= =Cancel= gives focus back to the previous contorl gives focus back to the previous control.
...
Aside from =Caption=, =BorderStyle= and =Aligment= may be useful for presentation.
=WordWrap= for multiline support or =AutoSize= to accomodate longer capture strings.
...
has to rely on =ItemData= to manually store actual data items, indexed by the same index of =ListBox.List=.
