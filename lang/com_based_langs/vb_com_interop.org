...
- =StrPtr=: because in VBA, a string has no indexer, one has to use a dedicated function =StrPtr= to
  obtain the address of the first element, instead of =VarPtr(str(0))=, which only works for an
  array.

- =ObjPtr=: =VarPtr= takes the address of a variable, =ObjPtr= takes the address of the object
  referenced by the variable.

* Dynamic Invocation with =DispCallFunc=

=DispCallFunc= takes all arguments in an array of pointers to variants,
that is an array of long integers, which are pointers to variants, which themselves
store the actual argument values. These values, if they are pointers, should also be referenced
(with =VarPtr=) before assigning to a variant. That is, for a pointer argument, two =VarPtr= is
required to set the argument pointer array. Also, the called-on object variable must be of the correct
interface type before =ObjPtr= to actually obtains its pointer value.
