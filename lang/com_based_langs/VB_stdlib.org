...
- =/= always converts its operands into =Double=; Use =\= for pure integer
  division.
  - VBA uses round-to-zero integer division.
...
- =Int=: round to the lower integer, a floor function; =Fix=: truncates the
...
- =Left$=, =Right$=, =Mid$=: substring; =Mid$= also returns a string slice (Mid statement),
