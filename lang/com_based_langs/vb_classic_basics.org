...
Sometimes these words can't even appear as part of a token.

Read [[https://www.aivosto.com/articles/obscure-vb.html][Obscure and Obsolete VB syntax]] before blindly learning
any VB syntax.

* Help & Documentation

- Install the MSDN Library Visual Studio 6.0, which holds most of the authoritative documentation. 
  One can simply click =Help= menu option and the help page for that entry (method, property, etc) pops up.

- The Archived Online [[https://learn.microsoft.com/en-us/previous-versions/visualstudio/visual-basic-6/aa232759(v=vs.60)][site]].
  + Not the best experience but contains a few articles not in the original MSDN library.

- [[https://learn.microsoft.com/en-us/office/vba/api/overview/language-reference][VBA Language Reference]]
  + Not the same with VB6 but basically the same language.
...
- =String= (=BSTR=): sequence of UTF-16 code points
  ...
  + =+= concatenation does work but it when used with an integer, it becomes arithmetic addition.
    Better use =&=
...
- =Currency= (=CURRENCY=, 8-byte number in an integer format scaled down by
  ...
  + literal suffix =@=
  + this can replace =int64= in some occasions but we don't know the exact semantics of its operations.
...
- Special values
  ...
  + =Error= (an error code): from 0 to 65535 Windows =HRESULT=, custom errors are defined by adding
    =vbObjectError= to custom error numbers to avoid conflicts.
...
- =Variant= can be useful when returning a result of different types on some
  ...
  =vbError= type, created with =CVErr=. checked using =IsError=.
...
+ UDTs in VB are limited in that if they are used with classes, they must be
  declared in a public object module, that is, a DLL or ActiveX EXE (standard EXE is not a COM server) 
  class module so that they may be marshaled by COM.

+ Array assignment follows value semantics.
...
=SAFEARRAY=: safe that in thread-safety, explicit metadata (a few flags on its memory location,
element type, size and bounds). Size and bound info along with dedicated access functions make it 
safe to use.
...
+ The number in =Dim arr(num) As TypeName= is the upper bound, the actual length is =num - 1=.

+ max size in a =Long= variable
...
+ Array can be assigned to a =Variant= with ~var = arr()~ with value semantics.
    - an array and an =Variant= can be assigned to each other.
    - an array in =Variant= has its =VarType(v) = vbArray + vbElementType=.
    - Array assignment uses value semantics even if the destination is a =Variant= variable.
      Basically, arrays use value semantics with assignment. ByRef parameter is not
      assignment anymore, it's parameter binding. 
    - The only way to implement a reference-based array is to create an array wrapper of =Object=.

+ Array assignment only works if the target is a dynamic array and both syntax (w/o =()=)
  forms are copy assignment.
  + one cannot alias an array by assignment.
...
Public [Shared] ' Shared is not Public, it means shared among module procedures, which is already the case for VBA
...
Dim a As Long: a = 10 ' Now we can declare and initialize on the same line
#+end_src
...
- Setting an =Object= variable requires a =Set=. Value assignment might
  ...
  + As long as reference assignment is
    involved, =Set= is required (because otherwise it's =Let=-assignment), even if the both sides
    are variants. Also, COM is refcounted,
    with a =Set= the compiler can't insert a =AddRef= call. One has to check using =IsObject= before
    assignment if the source can be an object.
...
- /Global/ :: BAS module =Public= variables. application long lifetime; project-wide scope
...
- /Public/ module-level :: except in BAS modules where /Public/ variables are
  global or called /property/; module-long (class, form) lifetime.
...
- /Static Local/ :: procedure-wide scope; module-level lifetime.
  ...
  + =Static= may precede or follow a procedure declaration to indicate all
    variables declared within the procedure have module extent. THIS IS NOT the =static= in common
    OOP languages.
...
- Logical operator with ===, =<>=; =And=, =Or=, =Xor=, =Not= bitwise operator
  ...
  + =Eqv=: performs a logical equivalence on two expressions, not limited to boolean types 
    but also integers (bitwise comparison) and even =Null=, this is similar to =Not (A Xor B)=.
...
- =GoSub=, =Return=, =On .. GoSub=: a primitive form of procedure call, where
  ...
  This is the only to call and implement a subroutine before =Sub= and =Function= keywords
  are introduced.
...
'                              and later Set-asigned to the next element of the collection, that is, 
'                              referencing that element.
...
- Public procedures of a public procedural module cannot be called through COM. =Public= is
...
parentheses. Otherwise, =Call= must be used to include arguments in parentheses. To avoid all these
...
- Parameters can be passed =ByVal= or =ByRef=
  ...
  + A =ByRef= parameter name stores the temporary value of a rvalue, the same as =ByVal= in this
    case.
  + For a =Variant= return value or similar =Variant=-to-=Variant= assignment where the source may
    be an object, use a =Sub LetSet(ByRef dst As Variant, ByRef src As Variant)= to handle it.
    Otherwise, we would need to evaluate our source expressions twice (erroneous) or get stuck in a
    catch-22 situation.
  + For =IUnknown= (=vbDataObject=) =Let= and =Set= yield  the same valid result. Prefer =Set=
    due to its reference nature (=AddRef=) maybe.

- Passing User Defined Types is restricted
  + a =Public= UDT defined in a =BAS= module can only be passed into
    =Public= procedures defined in BAS modules. (Such UDT types are unknown to COM)
  + To use structures across the COM/OLE boundary, define a class module in place of the UDT 
    or define the UDT in a object/class module (the generated typelib contains the definition).
    See [[https://stackoverflow.com/questions/975982/user-defined-type-udt-as-parameter-in-public-sub-in-class-module-vb6][UDT in Class Module]].
...
- (Obscure) =On Error Goto -1=: clears the error that just occurred in preparation
  for a new one to occur. Useful in a error handler to handler error.
...
mainhandler:
  ...
  On Error GoTo -1 ' Clear the error, without it, nestedhandler will not be executed.
