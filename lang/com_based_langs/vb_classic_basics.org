...
- Setting an =Object= variable requires a =Set=. Value assignment might
  ...
  + As long as either side is an object variable =Set= is required, otherwise the object variable
    evaluates to its default property. However a variable containing an object reference assigned to 
    another variable does not require =Set=.
