...
- =Boolean= (2-byte signed binary 2's complement integer): =0= for =False= and =-1= for =True=
...
- =Date= ([[https://learn.microsoft.com/en-us/cpp/atl-mfc-shared/date-type?view=msvc-170][OLE =DATE=]], 8-bytes): one-second precision. Each integer represents a day.
...
- Special values
  ...
  + =Missing=: no value is passed to the delcared optional parameter.
...
+ UDTs in VB are limited in that if they are used with classes, they must be
  declared in a public object module, that is, a DLL class module so that they
...
- =VB_PredeclaredId=: if =True=,  the default instance
  variable is given the name of the class as its name (not a singleton, as the
  user may still =New= another instance).
...
- =Option Private=: the accessiblity of a module to other projects.
  + Compared to =internal= in C#
  + A class module has to be in an ActiveX project or a library project to be public
