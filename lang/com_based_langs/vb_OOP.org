...
- =VB_PredeclaredId=: if =True=,  the default instance
  ...
  + without =VB_GlobalNameSpace=, the default instance is not exposed globally.

- =VB_GlobalNameSpace=: if =True=, as a global class module, its default
  instance (if any) and all its members may be accessed via simple name expressions, i.e.
...
- =VB_Exposed=: class module accessibility of the type
  + =public class= or =internal class=

- =VB_Creatable=: whether external projects can create instances of the class,
  i.e. the accessibility of the default constructor
  + combined with =VB_Exposed= to control the creation.
  + =public= constructor or =internal= constructor
...
- =TypeOf ... Is ...=: TypeOf...Is expression is used to check whether the value type of a value is compatible with a
  given type.
  + exact type match
...
- =TypeName()=: the name of an object's class in the form of a string.
  + due to the dynamic nature of VB6, we can get the name of a type without referencing its
    containing project or server component (DLL, EXE). Combined with dynamic property/method access,
    one can use a type without referencing the type and even not know what the type contains.
...
- an interface should be set to =VB_Creatable = False= to prevent instantiation.
