...
- Technologies
  ...
  + *RPC*
    ...
    - /Marshaling/, the process of passing data from one context to another (not the same as serialization but broader)
    - /stub/
...
- Challenges of Distributed Applications
  + /Performance/: communication tends to be chunky. Remote property access and
    callbacks require expensive roundtrips. The best optimization 
    is to avoid making distributed calls whenever possible: /chatty vs. chunky trade-off/
...
- /Contexts/ :: a further division within an application domain where a common
  ...
  within it.
  + e.g. synchronization context (where only a single thread is allowed). Each
  appdomain contains at least one context and has a default context if not specifically required.
...
or *context-bound* (=System.ContextBoundObject=, TODO).
...
- /Object Activation/ :: Before an object instance of a remotable type can be accessed, it must be created and initialized by a process.
  ...
  + Marshal-by-value types are copied via serialization and activated upon
    deserialization.
  + A type's activation is determined by the configuration of the .NET Remoting infrastructure rather than the type itself.
...
- Metadata distribution
  ...
  + create a metadata assembly of stand-in class definitions with the same name as the server's implementation
    assembly's but empty implementation (not really used).
...
because the server application publishes the type at a well-known Uniform Resource Identifier (URI) before activating object instances. 
The server process hosting the remotable type is responsible 
for configuring the type as a well-known object, 
publishing it at a specific well-known endpoint or address, and activating instances of the type only when necessary.
#+end_quote
...
- /Singleton/ :: This singleton instance is activated when first accessed by a
  ...
  #+begin_src csharp
  // server registration
 RemotingConfiguration.RegisterWellKnownServiceType(typeof(MySingletonType), "SomeURI", WellKnownObjectModes.Singleton); 
 // client configuration
 RemotingConfiguration.RegisterWellKnownClientType(typeof(MySingletonType), "MyUri");
  #+end_src

- /Single Call/ :: a new instance of that type for every method invocation a
  client makes.
  + stateless, for more traditional RPC, but still an OOP object though with a very limited lifetime.
  #+begin_src csharp
  // server registration
 RemotingConfiguration.RegisterWellKnownServiceType(typeof(MySingleCallType), "SomeURI", WellKnownObjectModes.SingleCall); 
 // client configuration
 RemotingConfiguration.RegisterWellKnownClientType(typeof(MySingleCallType), "MyUri");
  #+end_src
...
RemotingConfiguration.RegisterActivatedServiceType(typeof(ClientSideActivatedType));
// client side configuration
RemotingConfiguration.RegisterActivatedClientType(typeof(ClientSideActivatedType), "http://someURL");
#+end_src
...
The client uses a proxy object to interact with the remote object by some IPC.
...
=MarshalByRefObject= is used mostly for cross-appdomain object reference.
...
- *Channel Sinks* :: plug-in points that allow access to the underyling messages
  ...
  mechanism to send messages to a remote object. Sinks are processors along the chain.
...
- Make types remotable: =Serializable= or =MarshalByRefObject=

- Choose a host appdomain: console app, winforms app, windows services, IIS, COM+
  + console app: great for development and debugging, requires explicit startup
  + winforms: with a GUI
  + windows service: automatic startup and constantly running
  + IIS: easy security configuration; HTTP only
  + COM+: distributerd transactions and object pooling.

- Choose an activation model

- Choose a channel and a port

- Choose how clients obtain the server's metadata

There last three steps can be configured via a configuration file.
...
In .NET Remoting, a callback to the client requires a separate channel, not the original one
(typically the client's TCP channel).
...
.NET Remoting uses =IMessage= objects throughout the Remoting architecture.
A few child interfaces are defined to further describe .NET Remoting messages of various types

- =IMethodMessage=: describes stack-based methods

- =IMethodReturnMessage=: describes all messages returning to the client.

- =IMethodCallMessage=: 

- =IConstructionCallMessage=: activates a client-activated object.

- =IConstructionReturnMessage=: as a response to object activation

* Proxies

.NETFX creates a =TransparentProxy= dynamically by reflecting on the metadata of the remote object to act as a standin.
TransparentProxy intercepts a method call on it and creates a =MessageData= to describe the method call
and then passes to the =RealProxy= to create the remoting message. =TransparentProxy= is created by the framework
and the user can only customize =RealProxy=.

Proxy customization does not require server-side cooperation. However, if both sides need customization, better use message
sinks.
