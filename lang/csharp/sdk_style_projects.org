...
- Dotnet SDK 3.0 introduces a new [[https://github.com/dotnet/msbuild/issues/4704][way]] to embed a resource file (=System.Resources.Extensions=). If the project
  ...
  (ms)build=. The old =msbuild.exe= would work as it uses the old way (=resgen.exe= with =BinaryFormatter=) to embed a file.
