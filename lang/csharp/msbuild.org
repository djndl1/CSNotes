...
- Download the last working mono msbuild [[https://github.com/mono/msbuild/releases/][here]]
  + better build one instead of this bootstrap msbuild. One can successfully build one on 
    =debian/1%16.10.1+xamarinxplat.2021.05.26.14.00-0xamarin2=
...
- replace =Roslyn= with a newer version (5.X no longer works).

- =System.Reflection.Metadata= 5.0.0 is required instead of a symlink to the one used by Roslyn. Download it from Nuget.
