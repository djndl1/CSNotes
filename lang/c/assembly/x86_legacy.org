...
are accessed through the =DS= segment and an offset register (=BX=, =DI= or
=SI=).

#+begin_src asm
mov al, [bx] // relative to DS
#+end_src
...
- =sub=, =sbb=: subtraction is done using the adder circuitry (not the =add=
  ...
  + Since =x - y= is computed as =x + (2^N - y)=. If =x= is greater than =y=,
    =x-y= would be positive and the result unsigned overflows. That is, if there is no
    borrow, the carry status is set. The borrow status is the negation of the
    carry status. *However, here with x86 the carry flag bit is used to indicate a
