...
- =idiv= (signed): special in that the numerator is always double size the denominator.
  ...
  + commonly used with cross-register signed extension instructions.
...
- =sar=: arithmetic shift operations
  + mostly used for division (rounding towards negative infinity)
...
prefix in effect at the time of the interrupt: multiple prefixes on interrupt
...
- =.= member access operator is supported to access a struct member easily
  without manually computing the offset.
