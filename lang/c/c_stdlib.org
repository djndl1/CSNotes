...
=time_t= is not necessarily the number of seconds (calendar time) since the Unix epoch but is
almost always the case (even On Windows). C11 provides =struct timespec= and =timespec_get()= for
