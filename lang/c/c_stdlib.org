* Before Anything

The standard library in C is badly designed, only there to maintain compatibility 
with code from the 70s and 80s on Unix.

#+begin_quote
The C stdlib is essentially an SDK for very simple 70's UNIX-style command line tools, but operating
systems have moved on, while the C standard library is unfortunately stuck in the past.
#+end_quote

* Introduction
  :PROPERTIES:
  :CUSTOM_ID: introduction
  :END:
  
** Macro Functions

for those library that are defined as functions, C standard library permits
implementations to provide a function-like macro of the same name in addition to the true
function. the macro might provide a faster implementation of a simple function or it might call a
function of a different name. To bypass such macros

#+BEGIN_SRC C
  double (*p)(double) = &cos; double a = p(b);
  double a = (cos)(b);

  #undef cos
  double a = cos(b);
#+END_SRC

** POSIX Safety

- /MT-Safe/: thread-safe, safe to call in the presence of other threads. It means data races between
  threads are avoided using certain methods or there is no data to share at the very beginning.
  + Not atomic or uses memory synchronization mechanisms. Sometimes it simply means that a function
    has no internal global state so that other threads will not interfere. The reason why thread
    synchronization or locks are used is simply to protect shared data. Without shared data, there
    is no need for mutex, locks, atomicity etc. Such functions are inherently thread-safe.

- /AS-Safe/: Async-Signal-Safe, safe to call from (within) asynchronous singal handlers, that is,
  reentrancy inside a signal handler.
  + the signal handler can also safely interact with the same data — without deadlocks, inconsistent
    states, or non‑reentrant behaviour.
  + these functions can set =errno= or floating-point environment. signal handlers that call
    functions that may set errno or modify the floating-point environment must save their original
    values, and restore them before returning.

- /AC-Safe/: Async-Cancel-Safe, safe to all when asynchronous cancellation is enabled. 
  + only =pthread_cancel=, =pthread_setcancelstate=, and =pthread_setcanceltype=.

This is all about reentrancy

- Planned same-thread reentrancy, triggered by direct/indirect call, e.g. recursion.

- Cross-thread (concurrency)

- Unplanned same-thread (signal, interrupt), triggered by asynchronous signal.

Thread safety protects against other threads. Async-signal safety
protects against signal handlereentrancy and consistency. Thread safety is the guarantee of correct
behaviour under cross‑threadreentrancy, while signal safety is the guarantee under same‑thread
reentrancy. And many functions
that satisfy the first catastrophically fail the second. Thread concurrent calls and signal handlers 
are not the only scenarios of reentrant calls: there are /recursive calls/, which may not may not be
thread safe or signal safe: they are safe to call recursively but not thread-safe or
signal-safe. Now we have a third safety: /recursion safety/. Recursive functions (planned,
cooperative form of reentrancy) can maintain data
consistency and integrity with well-planned execution flows, but concurrency or async-signals, both
unplanned and preemptive, ignores the design of the functions and deal with the data in an unexpected way, and thus
might corrupt the state.

A mutex only helps with thread safety. But signal handlers executed on the same thread either triggers
a deadlock on non-recursive locks, or reenters the critical section and sees the partially updated
state. That is, thread safety ≠ async-signal safety ≄ atomicity. A mutex  does not provide true atomicity
in the face of signals or interrupts on the same thread and exposes the gap between theread safety
and genuine atomicity.

Also, a TLS slot is only local in terms of cross-thread scenarios, in terms
of signal handlers, it is global. To be more local, a function-local data (saved in an activation
record, while a TLS data item in a thread control block record) is required.

A mutex = thread-level illusion of atomicity. And that's why =sig_atomic_t= is provided in C: An
integer type which can be accessed as an atomic entity even in the presence of asynchronous
interrupts made by signals. Signal masks are to signal handlers as mutexes are to concurrency.

#+begin_quote
Our default mental model is purely sequential — one operation after another, nothing touching the
data in between. Safety measures exist precisely because reality (interrupts, context switches, true
parallelism) violates that assumption, introducing unexpected mutations that our sequential
reasoning never accounted for.
#+end_quote

There is a similar case in .NET: a UI thread that blocks on an async =Task= that is supposed to
continue on the same UI thread. It seems =Wait()= on a =Task= does block on something similar to a
lock (a event): how can a thread wakes itself up while sleeping! Locks and events are designed for
a threads to wake another thread up, not itself. Here, a thread is a resource, not a execution flow.
There are two execution flows that try to use the same thread. A thread is not the same as a
EXECUTION FLOW, it's just a bookkeeping record. A blcok wait holds this resource exclusively while
=await= puts two execution flows in the same position (message handlers). Block waiting is like
holding a lock, an exclusive ownership of a resource. With some AI comment:

#+begin_quote
locking wait is an exclusive hold on the thread, just like a mutex lock is an exclusive hold on a
critical section. The thread is the resource, and the blocked code refuses to yield it.

The collapse happens because the exclusive mechanism (the lock, the thread’s scheduling state) is
not designed for same‑thread reentrancy:

    A non‑recursive mutex says: “Only the owner can enter.” But when the owner is the one trying to
    re‑enter as a signal handler, the lock doesn’t see “it’s still me” — it just sees “already
    locked.” Deadlock.

    A blocked thread says: “This execution context is occupied — no other message can run here until
    I wake up.” But the continuation needs that exact context to wake the thread up. The thread is
    waiting for the continuation, the continuation is waiting for the thread. Deadlock.

That's why recursive locks solve the first case: they explicitly track the owner thread and allow
re‑entry. The lock recognizes its owner and lets it in.

But recursive locks can’t solve the async‑thread deadlock, because the resource here is the thread’s
ability to run code. A thread can’t “re‑enter” itself while it is synchronously blocked (suspended
waiting for an OS event). Even if we could make the thread concept reentrant, the physical control
flow is stuck. The fix, as you said, is:

    Cooperative (non‑exclusive): Use await — the thread is released back to the dispatcher; the
    continuation is just another message. The thread is never held exclusively; it’s shared and
    event‑driven.

    Reentrant via context‑escape: Use ConfigureAwait(false) — the continuation runs on a different
    thread, so the UI thread remains free to do other things (or even to block) without deadlocking
    the continuation.

In both cases, you remove the same‑thread reentrancy demand. Either the resource is never held
exclusively, or the reentrant flow doesn’t need the same resource at all.

You've built a powerful unified model: all safety is about managing reentrancy into resources that
were thought to be exclusive, and the collapse is always a self‑deadlock when the reentrant flow is
routed back onto the resource that is already held.
#+end_quote

All corruption results from unexpected non-sequential execution.

- =lock= 
  + AS-Unsafe: a signal handler might call the same function with a non-recursive lock and causes a deadlock.
  + AC-Unsafe: a cancellation causes the function to fail to release a lock and any further attempt
    to obtain the lock results in indefinite wait.

- =corrupt=: internal data structures might be corrupted and misbahave due to signal interrupts or
  thread cancellation. These functions might take recursive locks for thread safety but not
  enough for function reentry in a signal handler to see the partially updated state. Using locks 
  only prevents other threads from seeing the partial updates, but not a signal handler raised on
  the same thread.
  
- =heap=: may call heap memory management functions such as =malloc= and =free=.

- =dlopen=: AS- and AC- unsafe due to the use of the dynamic loader and holding loader locks.

- =plugin=: 

- =timer=: 

** Windows

The current implementation (UCRT) implements most of C99 and POSIX.1.
POSIX functions and Microsoft-specific functions are prefixed with an underscore
to comply to the C++ standard's requirement for reserve names.

* Standard Language Additions
  :PROPERTIES:
  :CUSTOM_ID: standard-language-additions
  :END:

Certain Standard C libraries can be considered part of the language.
They provide standard definitions and parameterization that help make C
programs more portable. They must be provided by freestanding
implementations even the other libraries are not provided.

- =stddef.h=: Fundamental types defined by the language, additional
  basic types and convenience macros.
  [[https://unix.stackexchange.com/questions/451232/where-is-stddef-h-defined-in-linux][On why I cannot find stddef.h]]

#+BEGIN_SRC C
  #ifdef __cplusplus
  #define NULL 0L
  #else
  #define NULL ((void*)0)
  #endif

  #ifndef __PTRDIFF_TYPE__
  #define __PTRDIFF_TYPE__ long int
  #endif
  typedef __PTRDIFF_TYPE__ ptrdiff_t;

  #ifndef __SIZE_TYPE__
  #define __SIZE_TYPE__ long unsigned int
  #endif
  #if !(defined (__GNUG__) && defined (size_t))
  typedef __SIZE_TYPE__ size_t;

  size_t    // unsigned integer type returned by `sizeof`
  ptrdiff_t // signed integer type returned when subtracting two 
  pointers
  max_align_t //
  offsetof   // byte offset from the beginning of a struct type to specified member
#+END_SRC

- =stdbool.h= C99 supports Boolean arithmetic with the built-in type
  =_Bool=

#+BEGIN_SRC C
  #define true 1
  #define false 0
  #define bool _Bool
#+END_SRC

- =iso646.h=

#+BEGIN_SRC C
  #ifndef _ISO646_H
  #define _ISO646_H

  #ifndef __cplusplus // in C++, these identifiers are keywords

  #define and    &&
  #define and_eq &=
  #define bitand &
  #define bitor  |
  #define compl  ~
  #define not    !
  #define not_eq !=
  #define or     ||
  #define or_eq  |=
  #define xor    ^
  #define xor_eq ^=

  #endif

  #endif
#+END_SRC

- =errno.h=: error report

#+BEGIN_SRC C
  #define EPERM        1  /* Operation not permitted */
  #define ENOENT       2  /* No such file or directory */
  #define ESRCH        3  /* No such process */
  #define EINTR        4  /* Interrupted system call */
  #define EIO      5  /* I/O error */
  #define ENXIO        6  /* No such device or address */
  #define E2BIG        7  /* Argument list too long */
  #define ENOEXEC      8  /* Exec format error */
  #define EBADF        9  /* Bad file number */
  #define ECHILD      10  /* No child processes */
  #define EAGAIN      11  /* Try again */
  #define ENOMEM      12  /* Out of memory */
  #define EACCES      13  /* Permission denied */
  #define EFAULT      14  /* Bad address */
  #define ENOTBLK     15  /* Block device required */
  #define EBUSY       16  /* Device or resource busy */
  #define EEXIST      17  /* File exists */
  #define EXDEV       18  /* Cross-device link */
  #define ENODEV      19  /* No such device */
  #define ENOTDIR     20  /* Not a directory */
  #define EISDIR      21  /* Is a directory */
  #define EINVAL      22  /* Invalid argument */
  #define ENFILE      23  /* File table overflow */
  #define EMFILE      24  /* Too many open files */
  #define ENOTTY      25  /* Not a typewriter */
  #define ETXTBSY     26  /* Text file busy */
  #define EFBIG       27  /* File too large */
  #define ENOSPC      28  /* No space left on device */
  #define ESPIPE      29  /* Illegal seek */
  #define EROFS       30  /* Read-only file system */
  #define EMLINK      31  /* Too many links */
  #define EPIPE       32  /* Broken pipe */
  #define EDOM        33  /* Math argument out of domain of func */
  #define ERANGE      34  /* Math result not representable */

  ...
#+END_SRC

An thread-local =int ``errno= holds implementation defined error codes
from library routine and is never cleared by library routines.

#+BEGIN_SRC C
  /* The error code set by various library functions.  */
  extern int *__errno_location (void) __THROW __attribute_const__;
  # define errno (*__errno_location ())
#+END_SRC

The typical way of using =errno= is to clear it before calling a library
function and check it afterwards. =strerror=, =perror= interpretes
=errno=.

- =stdarg.h=: a portable way to access variable argument lists

#+BEGIN_SRC C
  TYPEDEF __builtin_va_list va_list;

  #define va_start(v,l)   __builtin_va_start(v,l) // initializes a va_list
  #define va_end(v)       __builtin_va_end(v)     // called after all the argument have been read

  #define va_arg(v,l)     __builtin_va_arg(v,l)  // returns the value of the next parameter in the argument
  #define va_copy(d,s)    __builtin_va_copy(d,s) // duplicates the current state of a va_list
#+END_SRC

=va_copy= can be used to retain a pointer into the argument list

* Standard I/O

I/O functions are divided into two classes (called /orientation/): wide and
narrow.
A file stream is either wide or narrow. The first I/O operation determines its
orientation unless reopen. Before that, a stream is unbound.

The conversion between wide and narrow streams are handled within libc, affected
by =LC_CTYPE= upon orientation initialization, altered by =setlocale=.

The =FILE= object stores the state of a stream, including the error condition,
an EOF indicator, file position, orientation, read/write access rights,
conversion state, shift state (conversion), a file buffer. A =FILE= object is
not meant to be altered or copied.

Note that ANSI C requires that a file positioning function intervene between
output and input, unless an input operation encounters end-of- file.  (If this
condition is not met, then a read is allowed to return the result of writes
other than the most recent.)  Therefore it is good practice (and indeed
sometimes necessary under Linux) to put an fseek(3) or fsetpos(3) operation
between write and read operations on such a stream. *Linux does not mandate this
but it is always recommended so*.

** Remarks

- Text mode and binary mode

  #+begin_quote
Besides destroying your data, text streams have terrible performance. Opening everything in binary
mode is a simple enough work around, but standard input, output, and error are opened as text
streams, and there is no standard function for changing them to binary streams.
  #+end_quote

- Mutex?
  
  #+begin_quote
Typical implementations assume streams will be used by multiple threads, and so every access goes
through a mutex. This causes terrible performance for small reads and writes — exactly the case
buffering is supposed to most help.
  #+end_quote

Some of the libc I/O functions were designed based on record-oriented I/O on mainframe systems, weird for
Unix-style byte-oriented I/O programming.

** Pitfalls

- for =printf=-family functions
  #+begin_quote
If any argument is not the correct type for the corresponding conversion specification, the behavior is undefined.
  #+end_quote

* String and Character Processing
  :PROPERTIES:
  :CUSTOM_ID: string-processing-string.h-stdlib.h
  :END:
** Null Terminated String

 Due to historical consideration of performance, C chose to use null-terminated
 character array as the default string type (in constrast to alternatives such
 as length-prefixed Pascal P-String). In retrospect, this caused a few
 memory-related security, performance and design issues:
- no memory is allocated for the final =NUL= character and the =NUL= is written
  over adjacent memory, potentially causing data corruption or invalid memory access.
- no =NUL= is written.
- Inability to store zeros within a string.
- =O(n)= cost to find the length.
  + e.g. performance issue with =strcat=.
- functions are duplicated to handle text strings and binary data differently.

** Encoding and Character Set

=char= are uninterpreted bytes and =wchar_t= are implementation-defined and they
are subject to compiler options. Recent
C has added =char8_t= (UTF-8, C23), =char16_t= (16-bit character, typically UTF-16
but not necessarily, C11; C23 mandates UTF-16), =char32_t= (32-bit character, typically UTF-32 but not
necessarily, C11; C23 mandates UTF-32), along with their literal prefix =u8=, =u=, =U= (C11).

Character sets are discussed in terms of input character set (of the source
file), *source* character set (of the internal representation before feeding into
the compiler), *execution* character set (how characters are stored in the
binary). String and character literals stored in the input character set are
first converted into the source character set after reading, preprocessed and
then converted into the execution character set during compilation.

Each character set further contains the *basic* character set (basically ASCII)
and the *extended* character set.

ISO C requries a C program start with the "C" locale, which is meant to be the
simplest locale may or may not support UTF-8 and is probably restricted to ASCII.

** Remark

#+begin_quote
mblen, mbtowc, mbtowc, wctomb, mbstowcs, and wcstombs are connected to the locale system and don’t
necessarily operate on any particular encodings like UTF-8, which makes them unreliable. This is the
case for all the other wide character functionality, which is quite a few functions.
#+end_quote

** String Types

- Byte string :: each byte is assumed to be one character in some encoding.

- Multibyte string :: an extension to byte string. Locale-dependent character
  string that is layout-compatible with byte strings and uses the same
  facilities except for length calculation.
  + multibyte strings may be state-dependent encoding: beginning in an *initial
    shift* and enters *shift states* when specific multibyte characters are
    encountered in the sequence. The interpretation for subsequent bytes in the
    sequence is a function of the current shift state.

- Wide string :: implementation-defined wide-length character string, typically
  UTF-16 or UTF-32.

** Character Processing =<ctype.h>= =<wctype.h>=
  :PROPERTIES:
  :CUSTOM_ID: character-processing-ctype.h-wctype.h
  :END:

There are two kinds of facilities for handling characters:
classification (=is*=)and conversion (=to*=).

Every character conversion facility has a name beginning with =to= and returns a
value of type =int= representing a character or =EOF=. =EOF= is not necessarily
distinguishable from a real character if nonstandard character values
appear (Standard character values are always non-negative even if the
type =char= is signed). The narrow version accepts =int= but accepts only byte characters.

Character utiltity functions are dependent on the current locale.

Remarks:  the interface is misleading with an integer, which with some implementations
crashing on negative values, and these functions should probably be banned.
The locale-dependent behavior is mostly unexpected and undesirable. Write your own
for your own charset.

#+begin_src c
bool xisdigit(char c)
{
    return c>='0' && c<='9';
}
#+end_src


** String Concatenation

=strcat= and =strncat= are both dangerous to use since it is easy to write across
the end of the =dest= buffer and inefficient in performance
due to constant length searching to find the end of =dest= to append the =src=
($O(n^2)$ for n times of concatenation).
For a simple concatenation, this is acceptable but not for multiple string
concatenation since we already know the end of the last concatenation result.
Track the end of the intermediate result string and =memcpy= the next string to
the end of the imtermediate result to avoid constant searching from the start.
Think twice before using =strcat=.

BSD's =strlcat()= and the C11 extension =strcat_s= prevent writing off the end
of the destination buffer with different behavior.

Another approach to concantenate strings is to use =snprintf()=.
More automatic approaches include using string IO e.g. =open_memstream()=, which
handles memory allocation automatically.

** String Comparison

- =strcmp=, =strncmp=

- =strcpy=, =strncpy= (it may add additional =\0=)

#+BEGIN_SRC C
  char *strcat(char *dest, const char  *src)
  {
      char *s = dest + strlen(dest);
      strcpy(s, src);
      return dest;
  }
#+END_SRC

- =strlen=

- =strchr=, =strrchr=: finds the first/last occurrence of a character

#+BEGIN_SRC C
  int how_many(const char *s, int c)
  {
      int n = 0;
      if (c == 0) return 0;
      while (s) {
          s = strchr(s, c);
          if (s) n++, s++;
      }
      
      return n;
  }
  ``

  - `strspn`, `strcspn`, `strpbrk`: search a null terminated string for occurrences of characters specified by whether they are included in a second NTBS.

  ```c
  #include <stdbool>
  #include <stddef.h>
  #include <string.h>

  int is_id(const char *s)
  {
          static char *id_chars =
                  "abcdefghijklmnopqrstuvwxyz"
                  "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
                  "0123456789_";
          if (s == NULL) return false;
          if (strspn(s, id_chars) != strlen(s)) return false;

          return !isdigit(*s);
  }
#+END_SRC

- =strtok= (finds the next token), =strstr=: C++ =string.find()=

#+BEGIN_SRC C
          char input[] = "A bird   came down the walk";
          printf("Parsing the input string '%s'\n", input);
          char *token = strtok(input, " ");
          while(token) {
                  puts(token);
                  token = strtok(NULL, " ");
          }
#+END_SRC

- =strcoll, strxfrm=: provide locale-specific string-sorting facilities.

- =atof=, =atoi=, =atol=, =atoll=: convert the initial portion of a
  string to numbers

- =strtod=, =strtof=, =strtold=; =strtol=, =strtoll=, =strtoul=,
  =strtoull=: These functions provide more control over conversions than
  the corresponding facilities of =sscanf=.

* Memory Functions
  :PROPERTIES:
  :CUSTOM_ID: memory-functions
  :END:

In STD C, memory is interpreted as an array of objects of type
=unsigned char=. Block of memory are designated by a pointer of type
=void *=.

- =memchr=: searches for the first occurrence of a value in the first
  =len= characters beginning at a certain place

- =memcmp=: compare the first few bytes of two memory block

- =memcpy=, =memmove=: the difference is that =memmove= will work
  correctly for overlapping memory regions.

- =memset=: copies a value into a certain number of bytes beginning at a
  memory location.

** Windows

Aligned allocations are not supported. Use =_aligned_malloc=.

* Time Support

** Concept

Defined by glibc documentation.

- *calendar time*, *absolute time* :: a point in the time continuum.

- *interval*, :: a contiguous part of the time continuum between two
  calendar times
  + *elapsed time* :: the length of an interval
  + *period* :: an interval between two events

- *simple calendar time* :: a calendar time represented as an elapsed time since
  a fixed, implementation-specific calendar time (the *epoch*).

- *broken-down time* :: a calendar time represented by its components in the
  Gregorian calendar, relative to a specific time zone.

- *CPU time* :: the amount of time that a single process has actively used a CPU
  to perform computations.

- *processor time* :: the amount of /any/ CPU has been in use by /any/ process.
  + a basic system resource indicator

** ISO

=time_t= (simple calendar time) is not necessarily the number of seconds since
the Unix epoch (mandated by the POSIX) but is
almost always the case (even On Windows).

C11 provides =struct timespec= (a simple calendar time or an elapsed time) and =timespec_get()= for
high resolution calendar time (without any utility functions. One has to resort
to =time_t='s).

=ctime= and =asctime= format time in a fixed format, use =strftime= and
=wcsftime= instead.

- To break down a time, use =gmtime_*= (for UTC) and =localtime_*= (for local
  time)
  + there is no way in ISO C to convert a broken down UTC time back into a Unix time.

- Unix time -> local time, use =localtime_*=, use =mktime()= for the other way around.

- Note that =clock()= returns a CPU time, not a monotonic clock time.

** Unix

*** glibc

- =struct timeval= :: an older simple calendar time with microsecond resolution

The system may have several clocks of different resolutions, epochs and mechanisms.

- =clockid_t= :: system clock ID that represents several system clocks
  - used with =clock_gettime()=​/​=clock_settime()= and =clock_getres()=
  - =CLOCK_REALTIME= :: since the POSIX epoch
  - =CLOCK_MONOTONIC= :: useful for measuring elapsed times

=clock_settime()= sets the time of a certain clock. To avoid discontinuous
change in the calendar time of =CLOKC_REALTIME=, use =ntp_*= API to monitor and
discipline the clock.

glibc's =struct tm= is extended with an offset and a timezone name. It also
offers =timelocal()=, =timegm()= to extende ISO C.

In addition to =sleep()=, there is a thread-safe high-precision =nanosleep()=.

Timezone info is set by the =TZ= environment variable and =tzset()= into the
=tzname= variable. The timezone settings are process-global.

The [[man:time(1)][time]] program probably uses =times()= to measure CPU time.

** Windows

*** UCRT

=clock()= under UCRT does not conform to ISO C's CPU time requirement. It
returns wall clock time. Very limited on Windows, use =GetProcessTimes()= and
=QueryPerformanceCounter()= instead.

=_ftime*()= returns millisecond-level local time with timezone info.
=_mkgmtime()= is basically the Windows version of =timegm()=.

=_tzset()= is similar to POSIX =tzset()= but does not support the same =TZ= format.

* Signals

A signal is a notification to a process that an event has occurred. It may
result from a hardware exception, a user operation, a software event. It could be
used as a synchronization technique or a primitive form of IPC. After a signal
hander is executed, execution returns where it is interrupted.

=signal= is not required to be thread-safe but POSIX requires so.

Calling =abort= or =raise= forces the current execution to turn to a handler and
thus synchronous. Synchronous signal handlers cannot raise a signal again. An
asynchronous handler may not (undefined behavior) call most of the standard
library functions and access any static/thread-local object that is not
lock-free atomic (=sig_atomic_t= is atomic even in the presence of async signals)

Remarks: essentially useless on non-POSIX systems.

** Windows

=SIGINT= is not natively supported and =Ctrl-C= is handled by a new thread in the
process. =SIGILL= and =SIGTERM= are not generated by Windows but user code's
=raise=. =SIGINT= seems the only signal that can be generated externally to a process.

The handler for a signal is reset to the default before the custom
handler is executed. Signal dispositions are not inherited by child processes.

* Math Functions

They are inevitable and usually better implemented than a user's.

* Error Reproting

Most of the time the returned value simply indicates a failure but not the exact error
condition. Check =errno=:

** errno

Thread local on major platforms (POSIX, Windows and in many other even embedded systems) and
required so by the C11 standard and POSIX.

- Using =errno= requires the user to set it to =0= before calling a function so that the user would
  not be misled by previous =errno= values. Some functions set =errno= to nonzero even if they are
  not reporting any errors. 


- Error codes are not limited to the ones defined in POSIX: the user can define their own.

- =EWOULDBLOCK= and =EAGAIN= are of the same value.

- A proper signal handler should save and restore =errno=.

- C++ defines many of the POSIX error codes even if C does not.

Remarks: Terrible thread-local (or global) pseudo-variable. Better return a value. It also requires
a TLS implementation unless it is some very primitive non-conformant system.

*** Error Messages With =errno=

- =strerror=: terribly unsafe design, the returned error string is not a static one and is subject to
  invalidation by further calls (standard, POSIX and Windows). That's why there are thread-safe =strerror_s=
  (Windows) and =strerror_r= (POSIX, GNU)

A simple test (setting =errno= on different threads) with MINGW on MSVCRT reveals it it already
thread-safe since msvcrt.dll (VC++ 6.0) =errno= in MSVCRT is a macro that dereferences a pointer to
a possibly TLS slot.

#+begin_src c
#define errno (*_errno())
#+end_src

* Atomics

Types are not atomic: their operations are. C11 introduces this in a very non-custimizable OOP manner.

* Threads

Never really portable or fully featured. Even pthreads are more widespread, on Windows as well.
