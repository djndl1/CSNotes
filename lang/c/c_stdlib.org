...
Thread local on major platforms (POSIX, Windows and in many other even embedded systems) and required so by the C11 standard and POSIX.

Using =errno= requires the user to set it to =0= before calling a function so that the user
would not be misled by previous =errno= values. Some functions set =errno= to nonzero even if
they are not reporting any errors. Error codes are not limited to the ones defined in POSIX:
the user can define their own.

C++ defines many of the POSIX error codes even if C does not.

Remarks: Terrible thread-local (or global) pseudo-variable. Better return a value.

** Windows

A simple test with MINGW on MSVCRT reveals it it already thread-safe since msvcrt.dll (VC++ 6.0)
=errno= in MSVCRT is a macro that dereferences a pointer to a possibly TLS slot.
