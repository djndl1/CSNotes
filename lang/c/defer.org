...
Existing error handling and resource management relies heavily on 
error codes, out parameters, unreadable nested if blocks or potentially 
dangerous =goto=.

** C++

exception mechanism and RAII.
Defer meachanism is also easily implemented (e.g. =Boost.ScopeGuard=).

** Dlang

=try-finally=, scoped guard statements.
