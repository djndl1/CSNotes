...
In typical C and C++ practice, internal types, functions, variables are hidden by excluding the
declarations/definitions from   the public headers, anonymous namespace, =static= keyword,
=internal=/=detail= namespace, pimpl, interface-based API. However, real encapsulation come from
symbol visibility configuration, this is what .NET =public=/=internal= does (reflection aside) at
the assembly level.
