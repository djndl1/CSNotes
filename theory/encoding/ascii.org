...
- =EOT=, =0x04=, =^D= :: end of transmission, may place terminals on standby.
  ...
  + On modern systems, there isn't really a character used in processing or storage 
    that indicates this EOT or EOF, rather, EOT is now more a condition or a signal.
    EOTs are used to trigger the condition in terminal communication rather than 
    being embedded in files as a universal end marker.
