...
Individual user accounts (Windows users, groups or SQL server login, etc.) for logging on to the database engine.
...
*account lockout threshold*, *reset account lockout counter after* are also enabled.
...
- =db_denydatawriter=: write-forbidden

Combined with =DENY= asnd =GRANT=, one may create a role based on existing roles.
