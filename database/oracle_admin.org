...
- look at =dba_segments= and =dba_extents= to find the high watermark of each object.

- enable row movement =alter table <table> enable row movement=

- shrink tables and rebuild indexes.
  #+begin_src sql
  alter table <table> shrink space cascade;
  alter index <index> rebuild online;

  alter index <index> rebuild partition <partition> online;
  #+end_src
...
- move/compact tables
  #+begin_src sql
    ...
    -- lob data
    alter table <table> move partition <partition> lob (lob_column) store as (tablespace <tablespace_name>);
  #+end_src

- purge the recycle bin =purge tablespace <tablespace>=

- resize the data file =alter database datafile '<filepath>' resize <filesize>;=
...
- /user account/: identified by a /user name/ with the following attributes
  ...
  + a user schema
    + to access another schema, the schema prefix is required, otherwise, define
      a synonym for that object or =alter session set
      current_schema=that_schema= or use a after-logon trigger to set the
      current schema.
...
and the server, with priority from the lowest to the highest

- Database default values set during creation

- *Initialization parameters* on the server

- *Environment variables* on the client

- Session NLS parameters with =ALTER SESSION=

- In certain SQL functions =TO_CHAR=

Default values are always used when evaluating expressions in
virtual columns, CHECK constraints, and fine-grained auditing (FGA) rules which
requires deterministic results for the during of their existence.
...
The language and territory used by the client application and the database
server and the client's charset for displayed data or entered data. The simplest
way to specify locale behavior for Oracle database software.

- components: =[language]_[territory].[charset]=
  + Language settings affect day and month names, default sorting sequence, reading
    direction.
  + Territory settings affect day and week numbering, date format, decimal character
    and group separator, currency symbols.
  + Each language has a default charset associated with it if the charset is not set.
  + all three are components are optional. Default values are =AMERICAN= for
    the language, a derived value from the language value for the territory and
    a default charset associated with language.

- =NLS_LANG= determines the default values for other detailed NLS parameters.
  Howevere, each of them can be set in the client environment or in the session to override the
  value set by =NLS_LANG=.
...
- =NLS_LANG= should reflect the setting for the client and correctly enable proper
    ...
    + (Unix) in case the OS charset is different from =NLS_LANG=, set =NLS_OS_CHARSET=.
...
**** Data Dictionary Views Related to NLS
...
- =V$NLS_VALID_VALUES=: valid NLS parameter values to choose

- =V$NLS_PARAMETERS=: session parameters in use

**** Other Find-Grained Parameters

- =NLS_LANGUAGE=: by default derived from =NLS_LANG=, also subject to the OS language.

- =NLS_TERRITORY=: Date format, decimal character and group separator, local
  currency symbol, IOS currency symbol, Dual currency symbol, first day of the
  week, first week of the year (ISO), credit and debit symbols IOS week flag, list separator

- =NLS_DATE_FORMAT=, =NLS_DATE_LANGUAGE=, =NLS_TIMESTAMP_FORMAT=,
  =NLS_TIMESTAMP_FORMAT=, =NLS_TIMESTAMP_TZ_FORMAT=, : used for
  =TO_CHAR=, =TO_DATE=,

- =NLS_CALENDAR=: Japanese Imperial, ROC Official, Persian, Thai Buddha, Arabic
  Hijrah, English Hijrah, Ethiopian

- =NLS_NUMERIC_CHARACTERS=: decimal character and group separator

- =NLS_LIST_SEPARATOR=: =,=, =.=, =;= or =:=

- =NLS_SORT=: a set of mathcing and comparison rules for character data
  + either =BINARY= or =collation_name=

- =NLS_COMP=: the comparison behavior and SQL operations whose determined
  collation is =USING_NLS_COMP=

- =NLS_NCHAR_CONV_EXCP=: whether conversion errors are reported between
  =NCHAR=-family and =CHAR= family types.
