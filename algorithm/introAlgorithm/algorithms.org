...
the output to solve a well-specified computational problem. An algorithm is said
to be correct if for each input instance, it halts with the correct output. Some
problems (NP-complete) may not have best solutions but there are some efficient
practical algorithms. Algorithms are like machine hardware: advanced algorithms
make computating faster.
...
4. Compound data are organized into *objects*, which are composed of attributes, ~A.length~. A variable representing an array or object represents a pointer/reference to the data representing the array or object. Null pointer is denoted as ~NIL~.
...
- /divide and conquer/: break the problem into several subproblems that are similar to the original problem but smaller in size. This involves three steps at each level of recursion:
    + /divide/ the problem into a numberr of subproblems that are smaller instances of the same problem.
    + /conquer/ the subproblems by solving them recursively. Solve the problem in a straightforward manner if the size is small enough.
...
- C# =MoreLINQ= uses a singly-linked list to implement a list of prepend/append
  operations to an =IEnumerable=. See [[https://github.com/morelinq/MoreLINQ/blob/master/MoreLinq/PendNode.cs][PendNode in MoreLINQ]]. Multiple =Item=​s are
  semantically prepended/appended to a =Source= but implementation-wise
  prepended to a singly-linked list. When the =IEnumerable= is being iterated
  over, prepended items are yielded first, appended items are saved for later
  yield (prepending and appending do not interfere with each other)
   and the source is yield before the prepended items and the appended
  items. A =ConcatCount= is maintained to indicate the number of appended items.
...
Input: a sequence of =A[1...n]= of numbers

Output: permutation =B[1...n]= of =A= such that 
$B[1] \leq B[2] \leq \cdots \leq B[n]$.

A real sorting problem sorts a collection of /records/, which can be further
divided into /keys/ (sorted value) and /satellite data/.

** Overview

Sorting is essential to many practical problems and algorithms.
...
/shortest-path distance/ $\delta (s, v)$: the minumum number of edges in any
path from vertex $s$ to vertex $v$, if none $\delta (s, v) = \infty$.

- Lemma 1: Let $G=(V,E)$ be a directed or undirected graph, and let $s\in V$ be
an arbitrary vertex. Then, for any edge $(u,v)\in E$,

$$
\delta\left(s,v\right)\leq\delta\left(s,u\right)+1
$$

Proof: consider $u$ is reachable from $s$ and unreachable.

- Lemma 2: Let $G=(V,E)$ be a directed or undirected graph and suppose that BFS
  is run on $G$ from a given source vertex $s\in V$. Then upon termination, for 
  each vertex $v\in V$, the value $v, d$ computed by =BFS= satisfies 
  $v.d \geq \delta (s, v)$.

Proof: the inductive hypothesis is that $v.d \geq \delta (s,v)$ for all $v \in V$
and use Lemma 1 and that $v.d$ is never modified twice.

- Lemma 3: Suppose that during the execution of =BFS= on a graph $G=(V,E)$, the 
  queue $Q$ contains the vertices $\left\langle v_{1},v_{2},\dots,v_{r}\right\rangle$
  where $v_1$ is the head of $Q$ and $v_r$ is the tail. Then $v_{r}.d\leq v_{1}.d+1$
  for $i = 1, 2,\dots ,r-1$.

Proof: TODO

The $d$ values at the time that vertices are enqueued are monotonically increasing
over time.

- Corrollary: Suppose that vertices $v_i$ and $v_j$ are enqueued during the execution
  of =BFS=, and that $v_i$ is enqueued before $v_j$. Then $v_{i}.d \leq v_{j}.d$
  at the time that $v_{j}$ is enqueued.

Breadth-first search finds shortest-path distance.

- Theorem: Correctness of breadth-first search
  + Let $G=\left(V,E\right)$ be a directed or undirected graph, and suppose that
    =BFS= is run on $G$ from a given source vertex $s\in V$. Then, during its
    execution, BFS discovers every vertex $v \in V$ that is reachable from the 
    source $s$, and upon termination, $v.d=\delta\left(s,v\right)$ for all $v\in V$.
    Moreover, for any vertex $v \neq s$ that is reachable from $s$, one of the 
    shortest paths from $s$ to $v$ is a shortest path from $s$ to =v.pre= followed
    by the edge =(v.pre, v)=.
...
/predecessor subgraph/: For a graph $G=(V,E)$ with source $s$, the predecessor
 graph $G_{p} = (V_{p}, E_{p})$ where 

$$
V_{p}=\left\{ v\in V\colon v.p\neq\text{NIL}\right\} \cup\left\{ s\right\} 
$$

and

$$
E_{p}=\left\{ \left(v.p,v\right)\colon v\in V_{p}-\left\{ s\right\} \right\} 
$$

Lemma: When applied to a directed or undirected graph $G=(V,E)$, procedure =BFS=
constructs $.pre$ so that the predecessor subgraph $G_p = (V_p, E_p)$ is a 
breadth-first tree.

*** Depth-first search

Depth-first search explores edges out of the most recently discovered vertex $v$ 
that still has unexplored edges leaving it. Once all of $v$ 's edge have been 
explored, the search backtracks to explore edges leaving the vertex from which
$v$ was discovered. The process continues until all the vertices that are reachable
from the original source vertex have been discovered.
...
When developing a dynamic-programming algorithm, a sequence of four steps are 
followed in:
...
- definition: given a sequence $X=\left\langle x_{1},x_{2},\dots,x_{m}\right\rangle$
  the $i$-th prefix of $X$ is defined as $X_{i}=\left\langle x_{1},x_{2},\dots,x_{m}\right\rangle$.

- /Optimal substructure of an LCS/:Let 
  $X=\left\langle x_{1},x_{2},\dots,x_{m}\right\rangle$ and 
  $Y=\left\langle y_{1},y_{2},\dots,y_{n}\right\rangle$ be sequences, and let
  $Z=\left\langle z_{1},z_{2},\dots,z_{k}\right\rangle$ be any of LCS of $X$
  and $Y$.
...
An LCS of two sequences contains within it an LC S of prefixes of the two 
sequences.

**** Step 2. A recursive solution

To find an LCS of $X$ and $Y$, we need to find the LCSs of $X$ and $Y_{n-1}$
and $X_{m-1}$ and $Y$, doing either requires finding an LCS of $X_{m-1}$ and
$Y_{n-1}$, which is the /overlapping-subproblems property/ in the LCS problem.

Define $c[i, j]$ to be the length of an LCS of the sequences $X_{i}$ and $Y_{j}$

$$
c\left[i,j\right]=\begin{cases}
0 & \text{if }i=0\text{ or }j=0\\
c\left[i-1,j-1\right]+1 & \text{if }i,j>0\text{ and }x_{i}=y_{i}\\
\max\left(c\left[i,j-1\right],c\left[i-1,j\right]\right) & \text{if }i,j>0\text{ and }x_{i}\neq y_{i}
\end{cases}
$$
...
LCS-Length(X, Y)  
    m = X.length
    n = Y.length
    # b is the solution, c is its length
