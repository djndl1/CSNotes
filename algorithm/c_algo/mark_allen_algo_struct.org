...
Given integers $a_{1}, a_{2}, a_{3}, ..., a_{n}$, find the maximum value of 
$\sum_{k=i}^{j}a_{k}$. Negative sum not considered zero.
...
- The selection problem: the input is a list of $N$ elements, which can
  ...
  operations. The total running time is $O(N + \log N)$. Another way is
...
The general strategy to $h_k$ sort is for each position $i$, in$h_k, h_k+1, \dots, N-1$ 
place the element in the correct spot among
...
increment sequence is to use the sequence
 $h_{t} = \lfloor N/2 \rfloor$ and $h_k = \lfloor h_{k=1} / 2\rfloor$.
