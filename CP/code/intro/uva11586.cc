...
{
    int ref_cycle = min(dataset, signal_num);

    int ref_signal = ref_cycle * 2;
    while (ref_signal < 3600) {
        int lower_bound = ref_signal;
        int upper_bound = ref_cycle - 5 + ref_cycle;
...
int main()
{
    ...
    while (fgets(temp_line, 50, stdin) != NULL) {
        ...
        int green_time = find_simul_green(dataset, 10);
        if (green_time > 3600)
            printf("Set %d is unable to synch after one hour\n", case_num);
        else
            printf("Set %d synchs again at %d minite(s) and %d seconds"
